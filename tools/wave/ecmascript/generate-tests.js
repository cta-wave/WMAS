...
const excludedTests = [/15\.4\.4\.15-3-14\.js/, /15\.4\.4\.18-3-14\.js/, /15\.4\.4\.20-3-14\.js/, /15\.4\.4\.21-3-14\.js/, /15\.4\.4\.22-3-14\.js/]

let testCount = 0

const generateTest = async ({testsPath, outputPath, currentPath, templateContent}) => {
  ...
  if (stats.isDirectory()) {
    ...
    for (let file of files) {
      await generateTest({currentPath: path.join(currentPath, file), outputPath, testsPath, templateContent})
    }
  } else {
    if (currentPath.indexOf('.js') === -1 || excludedTests.some(regex => regex.test(currentPath))) {
      return
    }
    ...
    testContent = testContent.replace('{{TEST_URL}}', '/ecmascript/tests/' + jsRelativePath)
    testContent = testContent.replace('{{TEST_TITLE}}', jsRelativePath.split('/').pop())
...
(async () => {
  ...
  await generateTest({testsPath, outputPath: testsOutputPath, templateContent})
  await copyFile(adapterSourcePath, adapterDestinationPath)
  await copyDirectory(harnessDir, harnessOutputDir)
  console.log(`Generated ${testCount} tests in directory ${outputPath} (${path.resolve(outputPath)})`)
})()
